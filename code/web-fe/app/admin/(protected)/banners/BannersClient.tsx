...
function BannerForm({ banner, onSaved, onCancel }: { banner?: Banner; onSaved: () => void; onCancel: () => void }) {
  ...
  return (
    <form onSubmit={save} className="mb-6 p-5 rounded-xl border border-brand-lighter bg-brand-faint space-y-4">
      ...
      <div>
        <label className="block text-xs font-medium text-text mb-1.5">Image *</label>
        <ImageUploader
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} className={inputCls} placeholder="Summer Collection" />
        </div>
        <div>
          <label className="block text-xs font-medium text-text mb-1">Subtitle</label>
          <input value={form.subtitle} onChange={(e) => setForm((f) => ({ ...f, subtitle: e.target.value }))} className={inputCls} placeholder="Up to 20% off" />
        </div>
        <div>
          <label className="block text-xs font-medium text-text mb-1">Link URL</label>
          <input value={form.linkUrl} onChange={(e) => setForm((f) => ({ ...f, linkUrl: e.target.value }))} className={inputCls} placeholder="/products?categoryId=8" />
        </div>
        <div>
          <label className="block text-xs font-medium text-text mb-1">Position (order)</label>
          <input type="number" min="0" value={form.position} onChange={(e) => setForm((f) => ({ ...f, position: e.target.value }))} className={inputCls} />
        </div>
        <div>
          <label className="block text-xs font-medium text-text mb-1">Starts</label>
          <input type="date" value={form.startsAt} onChange={(e) => setForm((f) => ({ ...f, startsAt: e.target.value }))} className={inputCls} />
        </div>
        <div>
          <label className="block text-xs font-medium text-text mb-1">Ends</label>
